...
**Stage 2 (2026-07-21):** options P&L sim on REAL historical option bars (Alpaca serves expired contracts; NBBO history unavailable → spread MODELED as visible config). `pricing.ts` (strike grid + Friday expiry + price-band selection over the real chain) + `stage2.ts` (`simulateSwingExit` mirrors the live exit priority; portfolio sim with live caps + equity curve; sensitivity reruns at 1.5x/2x spread). Supports SBv2 (price-first, −50% stop) AND SBv1 (no mid-swing stop; EV picker approximated by price band — labeled). `getOptionBars` added to alpaca.ts (additive).
**Intraday engine (2026-07-21, SB 15M):** `intraday.ts` — completed-15m-candle replay of the full live gate stack + `simulateLadder` two-contract exit sim against REAL 15-minute option bars; spec §25 report (profit factor, T1/T2/breakeven/stop rates, by ticker/hour/score/direction/alignment); own `/backtest/[id]` view. Per-profile living strategy docs: `docs/strategies/{sbv1,sbv2,qqq-manual,sb15m}.md` — update in the same commit as any strategy change.
