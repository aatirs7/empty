...
**Stage 2 (2026-07-21, SBv2 only):** options P&L sim on REAL historical option bars (Alpaca serves expired contracts; NBBO history unavailable → spread MODELED as visible config). `pricing.ts` (strike grid + Friday expiry + SBv2's price-first $0.45-0.80 selection over the real chain) + `stage2.ts` (`simulateSwingExit` mirrors the live exit priority; portfolio sim with live caps + equity curve; sensitivity reruns at 1.5x/2x spread). `getOptionBars` added to alpaca.ts (additive). Per-profile living strategy docs: `docs/strategies/{sbv1,sbv2,qqq-manual}.md` — update in the same commit as any strategy change.
