...
- **Auto OFF by default** (`autoDefault:false`). Enable when ready: `npm run profile-auto -- sbv2 on` (off / buy-only also). `npm run profile-auto` prints all profiles' state. Verified: 48 valid SBv2 flip candidates scanned today; geometry + invalidations confirmed.
