// Living handoff doc. Rendered at /handoff with a copy button so it can be
// pasted into any chat to bring it up to speed. KEEP THIS UPDATED whenever
// architecture, features, or state change.
export const HANDOFF_UPDATED = "2026-07-06";

export const HANDOFF = `# Vega — Project Handoff
...
- P&L — total account balance card, goal bar, net (trade P&L minus API cost).
...
- Alpaca account: switching to a new \\$500 paper account ("vega", PA34D7UCJ09S) for realism (new API keys needed to point the app at it).
- Position sizing tuned for a small account: cheap OTM contracts (< maxContractPrice), buy multiple to fit perTradeBudget.
- Open direction from owner + a friend (Farrukh): cheap \\$1-2 contracts (done), \\$500 account (in progress), and a future upgrade to read charts/levels/patterns rather than only news (NOT built yet — highest-leverage next step).
