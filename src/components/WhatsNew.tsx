...
const VERSION = "2026-07-21-v20";

const UPDATES: { title: string; body: string }[] = [
  {
    title: "SB 15M rebuilt: the empty-space zone tap",
    body: "SB 15M is now Farrukh's new spec, replacing the old confirmation-candle version. It watches high-cap names travelling through EMPTY SPACE on the 15-minute chart (order blocks come from the 4-hour HTF setting — the chart itself never leaves 15 minutes) and trades the first touch of the boundary facing price: falling from empty space into the top of a zone below buys CALLS, rallying into the bottom of a zone above buys PUTS. The tap is the whole trigger — no confirmation candle, no structure read, no score, no model, no news check. \"Empty space\" is strict: the last completed 15-minute candle must sit entirely outside every active zone, and only the FIRST boundary in the direction price is travelling counts as an entry level. The touch has to land within a tenth of an ATR of that boundary, which rules out the bad versions of it: gapping clean through, already deep inside the zone, or chasing after price has bounced away. A stale data feed also blocks the trade. It buys ONE weekly contract at $1.00–$2.00, at or slightly in the money; the stop starts 20% below the actual fill; at +40% the stop moves to breakeven and it KEEPS HOLDING (that is not a profit-take); the whole contract sells at +100%; and everything is flat before the close, every day. Entries only between 9:45am and 2:45pm ET. Auto stays OFF until you add its account keys and review the paper results. Heads-up: an at-the-money weekly on a $200+ stock often costs more than $2, so expect it to skip those names rather than force a trade.",
  },
  {
